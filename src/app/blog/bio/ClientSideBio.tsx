...
export default function ClientSideBio({authors, articles}: { authors: Membre[], articles: PostData[] }) {
  const searchParams = useSearchParams();
  const searchAuthor = searchParams.get('auteur');
  const author = authors.find((author) => removeAccents(author.nom_membre) === removeAccents(searchAuthor!));

  const filteredArticles = articles.filter((article) => article.acf.auteur === author?.nom_membre);
    ...
    <>
      <section className="hero-banner filtered-hero">
        <div className={`blog-container`}>
          <h1>{author ? `Les articles de ${author.nom_membre}` : "Les articles de ..."}</h1>
          {author && <AuthorCard member={author} removeAccents={removeAccents} hideArticlesButton={true}/>}
        </div>
      </section>
      <section className="posts filtered-posts">
        <div className="blog-container">
          <h2 className="articles-title">Mes <span>Articles</span></h2>
